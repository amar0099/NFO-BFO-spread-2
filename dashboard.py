...
def get_atm_strikes():
    """Fetch latest spot and return ATM strikes for Sensex (nearest 500) and Nifty (nearest 50)."""
    try:
        token = get_shared_token()
        cid   = get_secret("FYERS_CLIENT_ID") or CLIENT_ID
        fyers = fyersModel.FyersModel(client_id=cid, token=token, log_path="")
        today = date.today().strftime("%Y-%m-%d")
        df_sx = fetch_candles(fyers, "BSE:SENSEX-INDEX", 1, today)
        if df_sx.empty: df_sx = fetch_candles(fyers, "BSE:SENSEX", 1, today)
        df_nf = fetch_candles(fyers, "NSE:NIFTY50-INDEX", 1, today)
        if df_nf.empty: df_nf = fetch_candles(fyers, "NSE:NIFTY50", 1, today)
        sx_spot = float(df_sx["close"].iloc[-1]) if not df_sx.empty else None
        nf_spot = float(df_nf["close"].iloc[-1]) if not df_nf.empty else None
        sx_atm  = int(round(sx_spot / 500) * 500) if sx_spot else None
        nf_atm  = int(round(nf_spot / 50)  * 50)  if nf_spot else None
        return sx_spot, sx_atm, nf_spot, nf_atm
    except Exception:
...
with tab2:
    ...
    _sx_spot, _sx_atm, _nf_spot, _nf_atm = get_atm_strikes()
    _sx_atm_d = _sx_atm or 80000
    _nf_atm_d = _nf_atm or 24800

    LEG_DEFAULTS = [
        ("BSE", "SENSEX", bse_exp, _sx_atm_d, "CE", 3.3),   # Leg 1 — Sensex
        ("NSE", "NIFTY",  nse_exp, _nf_atm_d, "CE", 1.0),   # Leg 2 — Nifty
        ("BSE", "SENSEX", bse_exp, _sx_atm_d, "CE", 3.3),   # Leg 3 — Sensex
        ("NSE", "NIFTY",  nse_exp, _nf_atm_d, "CE", 1.0),   # Leg 4 — Nifty
    ]

    _prev_atm = st.session_state.get("c_last_sx_atm")
    if "c_defaults_set" not in st.session_state or _prev_atm != _sx_atm_d:
        ...
        st.session_state["c_last_sx_atm"]  = _sx_atm_d

    _sx_spot2, _sx_atm2, _nf_spot2, _nf_atm2 = get_atm_strikes()
    if _sx_spot2:
        st.markdown(f"""<div style='font-size:11px;color:#64748b;background:#f1f5f9;
            border-radius:6px;padding:6px 12px;margin:4px 0 8px 0;font-family:monospace;'>
            Sensex spot: <b>{_sx_spot2:.0f}</b> → ATM <b style='color:#dc2626'>{_sx_atm2}</b>
            &nbsp;|&nbsp;
            Nifty spot: <b>{_nf_spot2:.0f}</b> → ATM <b style='color:#0284c7'>{_nf_atm2}</b>
    ...
    for row_idx in range(2):
        ...
        for j, i in enumerate(legs_in_row):
            ...
            with cols[off+4]: strike   = st.number_input("Strike",   value=float(st.session_state.get(f"c_str_{i}", float(_sx_atm_d if d_exch=="BSE" else _nf_atm_d))), step=100.0, key=f"c_str_{i}")
...
with tab1:
    ...
    _sx_spot1, _sx_atm1, _nf_spot1, _nf_atm1 = get_atm_strikes()
    if _sx_spot1:
        st.markdown(f"""<div style='font-size:11px;color:#64748b;background:#f1f5f9;
            border-radius:6px;padding:6px 12px;margin:4px 0 8px 0;font-family:monospace;'>
            Sensex spot: <b>{_sx_spot1:.0f}</b> → ATM <b style='color:#dc2626'>{_sx_atm1}</b>
            &nbsp;|&nbsp;
            Nifty spot: <b>{_nf_spot1:.0f}</b> → ATM <b style='color:#0284c7'>{_nf_atm1}</b>
        </div>""", unsafe_allow_html=True)
        # Auto-fill strike fields if not already set by user
        if "sx_ce_str" not in st.session_state: st.session_state["sx_ce_str"] = float(_sx_atm1)
        if "sx_pe_str" not in st.session_state: st.session_state["sx_pe_str"] = float(_sx_atm1)
        if "nf_ce_str" not in st.session_state: st.session_state["nf_ce_str"] = float(_nf_atm1)
        if "nf_pe_str" not in st.session_state: st.session_state["nf_pe_str"] = float(_nf_atm1)
    ...
    with legs_row[5]:  sensex_ce_strike  = st.number_input("CE Strike", value=float(st.session_state.get("sx_ce_str", _sx_atm1 or 80000)), step=100.0, key="sx_ce_str")
    with legs_row[6]:  sensex_pe_strike  = st.number_input("PE Strike", value=float(st.session_state.get("sx_pe_str", _sx_atm1 or 80000)), step=100.0, key="sx_pe_str")
    ...
    with legs_row[13]: nifty_ce_strike   = st.number_input("CE Strike", value=float(st.session_state.get("nf_ce_str", _nf_atm1 or 24800)), step=50.0, key="nf_ce_str")
    with legs_row[14]: nifty_pe_strike   = st.number_input("PE Strike", value=float(st.session_state.get("nf_pe_str", _nf_atm1 or 24800)), step=50.0, key="nf_pe_str")
