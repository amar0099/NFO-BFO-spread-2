...
with tab3:
    ...
    if iv_fetch:
        ...
        if fyers is None:
            ...
            if df_sx_s.empty or df_nf_s.empty:
                ...
                lo_sx    = round_to(last_sx, 500)
                if lo_sx > last_sx: lo_sx -= 500
                hi_sx    = lo_sx + 500
                lo_nf    = round_to(lo_sx / iv_mult, 50)
                hi_nf    = round_to(hi_sx / iv_mult, 50)
                ...
                # Fetch 8 option series
                sym_map = {
                    "sx_lo_CE": build_symbol("BSE", "SENSEX", sx_iv_expiry, "C", lo_sx),
                    "sx_lo_PE": build_symbol("BSE", "SENSEX", sx_iv_expiry, "P", lo_sx),
                    "nf_lo_CE": build_symbol("NSE", "NIFTY",  nf_iv_expiry, "C", lo_nf),
                    "nf_lo_PE": build_symbol("NSE", "NIFTY",  nf_iv_expiry, "P", lo_nf),
                    "sx_hi_CE": build_symbol("BSE", "SENSEX", sx_iv_expiry, "C", hi_sx),
                    "sx_hi_PE": build_symbol("BSE", "SENSEX", sx_iv_expiry, "P", hi_sx),
                    "nf_hi_CE": build_symbol("NSE", "NIFTY",  nf_iv_expiry, "C", hi_nf),
                    "nf_hi_PE": build_symbol("NSE", "NIFTY",  nf_iv_expiry, "P", hi_nf),
                }
                with st.spinner("Fetching option chains..."):
                    opt_dfs = {}
                    for key, sym in sym_map.items():
                        df_o = fetch_candles(fyers, sym, iv_interval, iv_date_str)
                        opt_dfs[key] = df_o[~df_o.index.duplicated(keep="last")] if not df_o.empty else df_o

                # Compute IV time series
                r = iv_rate / 100
                def iv_series(spot_df, opt_df, K, expiry_str, opt_type):
                    if opt_df.empty or spot_df.empty:
                        return pd.Series(dtype=float)
                    exp_dt = expiry_to_date(expiry_str)
                    common = spot_df.index.intersection(opt_df.index)
                    out = {}
                    for ts in common:
                        T = max((exp_dt - ts.date()).days / 365, 1/365)
                        out[ts] = calc_iv(opt_df.loc[ts,"close"], spot_df.loc[ts,"close"], K, T, r, opt_type)
                    return pd.Series(out, dtype=float)

                iv_res = {
                    "sx_lo_CE": iv_series(df_sx_s, opt_dfs["sx_lo_CE"], lo_sx, sx_iv_expiry, "CE"),
                    "sx_lo_PE": iv_series(df_sx_s, opt_dfs["sx_lo_PE"], lo_sx, sx_iv_expiry, "PE"),
                    "nf_lo_CE": iv_series(df_nf_s, opt_dfs["nf_lo_CE"], lo_nf, nf_iv_expiry, "CE"),
                    "nf_lo_PE": iv_series(df_nf_s, opt_dfs["nf_lo_PE"], lo_nf, nf_iv_expiry, "PE"),
                    "sx_hi_CE": iv_series(df_sx_s, opt_dfs["sx_hi_CE"], hi_sx, sx_iv_expiry, "CE"),
                    "sx_hi_PE": iv_series(df_sx_s, opt_dfs["sx_hi_PE"], hi_sx, sx_iv_expiry, "PE"),
                    "nf_hi_CE": iv_series(df_nf_s, opt_dfs["nf_hi_CE"], hi_nf, nf_iv_expiry, "CE"),
                    "nf_hi_PE": iv_series(df_nf_s, opt_dfs["nf_hi_PE"], hi_nf, nf_iv_expiry, "PE"),
                }
                st.session_state["iv_res"]  = iv_res
                st.session_state["iv_lo_sx"] = lo_sx; st.session_state["iv_hi_sx"] = hi_sx
                st.session_state["iv_lo_nf"] = lo_nf; st.session_state["iv_hi_nf"] = hi_nf
    ...
    if "iv_res" in st.session_state:
        ...
        def iv_chart(pfx, sx_k, nf_k, title):
            fig = go.Figure()
            entries = [
        ...
        st.plotly_chart(iv_chart("sx_lo", lo_sx, lo_nf, f"IV % — Sensex {lo_sx} & Nifty {lo_nf}"), use_container_width=True)
        ...
        st.plotly_chart(iv_chart("sx_hi", hi_sx, hi_nf, f"IV % — Sensex {hi_sx} & Nifty {hi_nf}"), use_container_width=True)
