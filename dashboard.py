...
with st.sidebar:
    st.markdown("<div style='font-family:Syne;font-size:20px;font-weight:800;margin-bottom:4px;'>⚡ Spread Terminal</div><div style='font-size:11px;color:#64748b;margin-bottom:20px;font-family:Space Mono'>NFO / BFO Options</div>", unsafe_allow_html=True)

    # Theme toggle
    col_t1, col_t2 = st.columns([3,1])
    with col_t1:
        st.markdown("<div style='font-size:12px;font-weight:600;padding-top:6px;'>🌙 Dark Mode</div>", unsafe_allow_html=True)
    with col_t2:
    ...
    show_raw     = st.checkbox("Show Raw Prices", value=False)
...
with st.expander("🔍 Active Symbols"):
...
if df.empty:
    ...
    if show_diff:
        fig.update_yaxes(gridcolor=T["grid"], title_text="4 Leg", tickfont=dict(size=10), row=2, col=1)
        fig.update_xaxes(gridcolor=T["grid"], tickfont=dict(size=10), row=2, col=1)
    ...
    if show_raw:
        ...
        fig2.update_layout(height=320, plot_bgcolor=T["plot_bg"], paper_bgcolor=T["plot_bg"], font=dict(color=T["text2"], family="Space Mono"), hovermode="x unified", margin=dict(l=10, r=10, t=10, b=10), xaxis=dict(gridcolor=T["grid"]), yaxis=dict(gridcolor=T["grid"], title="Price (₹)"), hoverlabel=dict(bgcolor=T["card"], bordercolor=T["card_bdr"]))
