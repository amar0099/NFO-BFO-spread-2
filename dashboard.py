...
def fetch_expiries_for(token, fyers_sym):
    """Fetch expiry list for a given index symbol. Cached per (token, symbol)."""
    try:
        cid = get_secret("FYERS_CLIENT_ID") or CLIENT_ID
        ...
        if resp and resp.get("s") == "ok":
            data = resp.get("data", {})
            raw = data.get("expiryData", []) or data.get("expiry", [])
            for entry in raw:
                if isinstance(entry, dict):
                    e = entry.get("expiry") or entry.get("date") or entry.get("expiryDate")
                    if e is not None: expiries.append(_parse_expiry(e))
                else:
                    expiries.append(_parse_expiry(entry))
        return expiries
    except Exception:
        return []

def get_expiries_for(exchange, underlying):
    """Get cached expiry list for a given exchange+underlying."""
    try:
        token  = get_shared_token()
        sym    = _UNDERLYING_SYM.get(underlying.upper())
        if not sym:
            # Fallback: construct index symbol from exchange+underlying
            sym = f"{exchange}:{underlying}-INDEX"
...
def expiry_selectbox(label, opts, manual_key, select_key, default_manual):
    ...
    return st.selectbox(label, options, key=select_key)
...
def fetch_live_data():
    ...
    if any(df.empty for df in [df_sx_ce, df_sx_pe, df_nf_ce, df_nf_pe]):
        st.warning(f"⚠️ No data for one or more symbols. Built: {sym_sx_ce} | {sym_sx_pe} | {sym_nf_ce} | {sym_nf_pe}")
        return pd.DataFrame()
...
with tab1:
    ...
    if _sx_opts:
        with st.expander("🔍 Debug: raw expiry values from Fyers", expanded=False):
            try:
                token = get_shared_token()
                _dbg_fyers = fyersModel.FyersModel(client_id=get_secret("FYERS_CLIENT_ID") or CLIENT_ID, token=token, log_path="")
                _dbg_sym = _UNDERLYING_SYM.get(sensex_underlying.upper(), f"{sensex_exchange}:{sensex_underlying}-INDEX")
                _dbg_resp = _dbg_fyers.optionchain(data={"symbol": _dbg_sym, "strikecount": 1, "timestamp": ""})
                _dbg_raw = _dbg_resp.get("data", {}).get("expiryData", []) if _dbg_resp else []
                st.write("Raw expiryData (first 3):", _dbg_raw[:3])
                st.write("Parsed opts:", _sx_opts[:5])
