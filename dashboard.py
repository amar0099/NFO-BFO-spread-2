...
def fetch_expiries_cached(token):
    ...
    def parse_expiry(e):
        """Convert epoch int or string to YYMMDD string."""
        try:
            epoch = int(e)
            dt = _dt.datetime.fromtimestamp(epoch, tz=_dt.timezone.utc)
            return dt.strftime("%y%m%d")
        except (ValueError, TypeError):
            return str(e).strip()  # already a string like "26MAR12"
            ...
            if resp and resp.get("s") == "ok":
                ...
                for entry in raw:
                    if isinstance(entry, dict):
                        ...
                        if e is not None:
                            expiries.append(parse_expiry(e))
                    else:
                        expiries.append(parse_expiry(entry))
            result[label] = expiries
        result["_ok"] = bool(result.get("SENSEX") or result.get("NIFTY"))
        return result
    except Exception as e:
...
def expiry_selectbox(label, opts, manual_key, select_key, default_manual):
    """Dropdown of fetched expiries + a manual text input below it."""
    if opts:
        sel = st.selectbox(label, ["-- manual --"] + opts, key=select_key)
        if sel == "-- manual --":
            return st.text_input("", value=default_manual, key=manual_key, placeholder="YYMMDD or 26MAR12")
        return sel
    else:
        return st.text_input(label, value=default_manual, key=manual_key, help="YYMMDD or 26MAR12")
...
with tab1:
    ...
    _exp = get_expiries()
    _sx_opts = _exp.get("SENSEX", [])
    _nf_opts = _exp.get("NIFTY", [])
    if not _exp.get("_ok") and _exp.get("_error"):
        st.caption(f"⚠️ Expiry fetch failed: {_exp['_error']}")
...
with tab3:
    ...
    _iv_exp  = get_expiries()
    _iv_sx_opts = _iv_exp.get("SENSEX", [])
    _iv_nf_opts = _iv_exp.get("NIFTY",  [])
