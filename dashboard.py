...
with tab1:
    ...
    if df.empty:
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df["pe_spread"], name="PE Spread",
            ...
            fill="tonexty", fillcolor="rgba(255,100,100,0.07)",
            hovertemplate="%{x|%H:%M}<br>PE: %{y:.2f}<extra></extra>"), row=1, col=1)
        fig.add_shape(type="line", x0=0, x1=1, y0=0, y1=0,
            xref="paper", yref="y", line=dict(color="#444", width=1, dash="dash"))
        ...
        if show_diff:
            ...
            diff_yref = f"y{diff_row}"
            diff_high = df["diff"].max()
            diff_low  = df["diff"].min()
            fig.add_shape(type="line", x0=0, x1=1, y0=0, y1=0,
                xref="paper", yref=diff_yref, line=dict(color="#444", width=1, dash="dash"))
            fig.add_shape(type="line", x0=0, x1=1, y0=diff_high, y1=diff_high,
                xref="paper", yref=diff_yref, line=dict(color="#ffaa00", width=1, dash="dot"), opacity=0.6)
            fig.add_annotation(x=1.01, y=diff_high, xref="paper", yref=diff_yref,
                text=f"H:{diff_high:.0f}", showarrow=False, xanchor="left", font=dict(color="#ffaa00", size=10))
            fig.add_shape(type="line", x0=0, x1=1, y0=diff_low, y1=diff_low,
                xref="paper", yref=diff_yref, line=dict(color="#ffaa00", width=1, dash="dot"), opacity=0.6)
            fig.add_annotation(x=1.01, y=diff_low, xref="paper", yref=diff_yref,
                text=f"L:{diff_low:.0f}", showarrow=False, xanchor="left", font=dict(color="#ffaa00", size=10))
        ...
        if has_synth:
            ...
            synth_yref = f"y{synth_row}"
            ratio_high = df["synth_ratio"].max()
            ratio_low  = df["synth_ratio"].min()
            fig.add_shape(type="line", x0=0, x1=1, y0=ratio_high, y1=ratio_high,
                xref="paper", yref=synth_yref, line=dict(color="#818cf8", width=1, dash="dot"), opacity=0.6)
            fig.add_annotation(x=1.01, y=ratio_high, xref="paper", yref=synth_yref,
                text=f"H:{ratio_high:.4f}", showarrow=False, xanchor="left", font=dict(color="#818cf8", size=10))
            fig.add_shape(type="line", x0=0, x1=1, y0=ratio_low, y1=ratio_low,
                xref="paper", yref=synth_yref, line=dict(color="#818cf8", width=1, dash="dot"), opacity=0.6)
            fig.add_annotation(x=1.01, y=ratio_low, xref="paper", yref=synth_yref,
                text=f"L:{ratio_low:.4f}", showarrow=False, xanchor="left", font=dict(color="#818cf8", size=10))
    ...
    if "iv_res" in st.session_state:
        ...
        def iv_chart(pfx, sx_k, nf_k, title, chg_sx=None, chg_nf=None):
            ...
            for key, name, color, width in entries:
                ...
                if not s.empty:
                    # Build custom hover with per-minute strike
                    sk_aligned = sk.reindex(s.index).ffill().fillna(0).astype(int)
                    hover_text = [f"%{{x|%H:%M}}<br>Strike: {k}<br>IV: {iv:.2f}%"
        ...
        # Graph 2
        st.markdown(f"<div style='font-size:12px;font-weight:700;color:#64748b;margin:10px 0 6px 0;'>📈 Graph 2 — Sensex ATM+500 &nbsp;<span style='color:#f87171'>{range_label(hi_sx_range)}</span> &nbsp;/ Nifty <span style='color:#60a5fa'>{range_label(hi_nf_range)}</span> &nbsp;<span style='font-weight:400;font-size:11px;'>(dynamic ATM · closing: {hi_sx}/{hi_nf})</span></div>", unsafe_allow_html=True)
        c1,c2,c3,c4 = st.columns(4)
        c1.markdown(card(f"Sensex ATM+500 CE IV ({range_label(hi_sx_range)})", iv_res.get("sx_hi_CE"), COLORS["sx_CE"], SENSEX_LOT), unsafe_allow_html=True)
        c2.markdown(card(f"Sensex ATM+500 PE IV ({range_label(hi_sx_range)})", iv_res.get("sx_hi_PE"), COLORS["sx_PE"], SENSEX_LOT), unsafe_allow_html=True)
        c3.markdown(card(f"Nifty ATM+500 CE IV ({range_label(hi_nf_range)})",  iv_res.get("nf_hi_CE"), COLORS["nf_CE"], NIFTY_LOT),  unsafe_allow_html=True)
        c4.markdown(card(f"Nifty ATM+500 PE IV ({range_label(hi_nf_range)})",  iv_res.get("nf_hi_PE"), COLORS["nf_PE"], NIFTY_LOT),  unsafe_allow_html=True)
        st.plotly_chart(iv_chart("sx_hi", hi_sx, hi_nf, f"IV % — Sensex ATM+500 {range_label(hi_sx_range)} & Nifty ATM+500 {range_label(hi_nf_range)}", iv_changes.get("hi_sx"), iv_changes.get("hi_nf")), use_container_width=True)
