...
st.set_page_config(page_title="NFO/BFO Spread Terminal", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
...
with st.sidebar:
    ...
    refresh_secs = st.slider("Refresh (sec)", 5, 60, REFRESH_SECONDS)
...
with tab2:
    ...
    EXCHANGES   = {"SENSEX": "BSE", "BANKEX": "BSE", "NIFTY": "NSE", "BANKNIFTY": "NSE", "FINNIFTY": "NSE", "MIDCPNIFTY": "NSE"}
    ...
    for i in range(4):
        ...
        cols = st.columns([1, 1.4, 1, 1, 1.2, 1.2, 0.9])
        ...
        with cols[2]: expiry = st.text_input("Expiry",    value="260312",                key=f"c_exp_{i}", help="YYMMDD")
        ...
        with cols[5]: action = st.selectbox("Action",    ["BUY","SELL"],      index=i%2, key=f"c_act_{i}")
        with cols[6]: lots   = st.number_input("Lots",   value=1, min_value=1,           key=f"c_lots_{i}", step=1)
        leg_configs.append({"exchange": exch, "underlying": under, "expiry": expiry,
                            "strike": int(strike), "opt_type": opt_type, "action": action, "lots": lots})

    c_date     = st.columns([1.2, 1, 3])
    with c_date[0]: custom_date = st.date_input("Date", value=default_date, key="c_date")
    with c_date[1]: custom_interval = st.selectbox("Interval", [1,3,5,10,15,30,60], index=2, key="c_interval")
    with c_date[2]: custom_fetch = st.button("⟳  FETCH 4-LEG DATA", type="primary", key="c_fetch")
    ...
    for i, leg in enumerate(leg_configs):
        ...
        formula_parts.append(f"{sign} {leg['lots']}×{leg['underlying']} {leg['opt_type']}")
    formula_str = "  ".join(formula_parts).lstrip("+ ").replace("+ −","− ")
    st.markdown(f"<div style='font-size:11px;color:#64748b;margin:4px 0 8px 0;font-family:monospace;'>Formula: {formula_str}</div>", unsafe_allow_html=True)
        ...
        if fyers is None:
            ...
            with st.spinner("Fetching 4-leg data..."):
                for i, leg in enumerate(leg_configs):
                    ...
                    dfs.append((df_leg["close"], leg))

            if ok and dfs:
                base_idx = dfs[0][0].index
                result = pd.Series(0.0, index=base_idx)
                for series, leg in dfs:
                    aligned = series.reindex(base_idx, method="ffill").fillna(0)
                    if leg["action"] == "BUY":
                        result += aligned * leg["lots"]
                    else:
                        result -= aligned * leg["lots"]
                st.session_state.df_custom = result.to_frame(name="value")

    df_custom = st.session_state.df_custom

    if not df_custom.empty:
        latest_val = df_custom["value"].iloc[-1]
        high_val   = df_custom["value"].max()
        low_val    = df_custom["value"].min()

        mc1, mc2, mc3 = st.columns(3)
        mc1.metric("Latest", f"{latest_val:+.2f}")
        mc2.metric("Day High", f"{high_val:.2f}")
        mc3.metric("Day Low",  f"{low_val:.2f}")

        fig_c = go.Figure()
        fig_c.add_trace(go.Scatter(
            x=df_custom.index, y=df_custom["value"],
            name="4-Leg P&L", line=dict(color="#818cf8", width=2),
            fill="tozeroy", fillcolor="rgba(129,140,248,0.08)",
            hovertemplate="%{x|%H:%M}<br>Value: %{y:.2f}<extra></extra>"
        ))
        fig_c.add_hline(y=0, line_dash="dash", line_color="#444")
        fig_c.add_hline(y=high_val, line_dash="dot", line_color="#34d399", line_width=1,
            annotation_text=f"H: {high_val:.0f}", annotation_position="right",
            annotation_font=dict(color="#34d399", size=10))
        fig_c.add_hline(y=low_val, line_dash="dot", line_color="#f87171", line_width=1,
            annotation_text=f"L: {low_val:.0f}", annotation_position="right",
        ...
        fig_c.update_layout(
            ...
            yaxis=dict(gridcolor=T["grid"], title="P&L (₹)", tickfont=dict(size=10),
                showspikes=True, spikemode="across", spikecolor=T["text3"], spikethickness=1, spikedash="dot"),
