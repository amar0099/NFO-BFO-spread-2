...
@st.cache_resource
def fetch_expiries_cached(token):
    """Fetch expiry list for SENSEX and NIFTY once per token."""
    try:
        cid = get_secret("FYERS_CLIENT_ID") or CLIENT_ID
        fyers = fyersModel.FyersModel(client_id=cid, token=token, log_path="")
        result = {}
        for sym, label in [("BSE:SENSEX-INDEX", "SENSEX"), ("NSE:NIFTY50-INDEX", "NIFTY")]:
            resp = fyers.optionchain(data={"symbol": sym, "strikecount": 1, "timestamp": ""})
            expiries = []
            if resp and resp.get("s") == "ok":
                data = resp.get("data", {})
                # Try expiryData list first
                for entry in data.get("expiryData", []):
                    if isinstance(entry, dict):
                        e = entry.get("expiry") or entry.get("date") or entry.get("expiryDate")
                        if e: expiries.append(str(e))
                    elif isinstance(entry, str):
                        expiries.append(entry)
                # Fallback: optionChain may have expiries as top-level list
                if not expiries:
                    for entry in data.get("expiry", []):
                        expiries.append(str(entry))
            result[label] = expiries
        result["_ok"] = True
        return result
    except Exception as e:
        return {"SENSEX": [], "NIFTY": [], "_ok": False, "_error": str(e)}

def get_expiries():
    """Get cached expiries for current token."""
    try:
        token = get_shared_token()
        return fetch_expiries_cached(token)
    except Exception:
        return {"SENSEX": [], "NIFTY": [], "_ok": False}
...
def build_symbol(exchange, underlying, expiry, option_type, strike):
    ...
    # If expiry contains letters it's already text format (e.g. 26MAR12 or 2026MAR)
    # Pass it through directly to Fyers
    if any(c.isalpha() for c in expiry):
        return f"{exchange}:{underlying}{expiry}{strike}{ot}"
    # Numeric YYMMDD → YYM DD (strip leading zero from month)
...
with tab1:
    ...
    _expiries = get_expiries()
    if not _expiries.get("_ok", True) and _expiries.get("_error"):
        st.warning(f"⚠️ Could not fetch expiries from Fyers: {_expiries['_error']}. Enter manually.")
    _sx_exp_list = _expiries.get("SENSEX", [])
    _sx_exp_opts = _sx_exp_list if _sx_exp_list else ["260312"]
    with r1[2]: sensex_ce_expiry  = st.selectbox("CE Expiry", _sx_exp_opts, key="sx_ce_exp")
    with r1[3]: sensex_pe_expiry  = st.selectbox("PE Expiry", _sx_exp_opts, key="sx_pe_exp")
    ...
    _nf_exp_list = _expiries.get("NIFTY", [])
    _nf_exp_opts = _nf_exp_list if _nf_exp_list else ["260310"]
    with r2[2]: nifty_ce_expiry  = st.selectbox("CE Expiry", _nf_exp_opts, key="nf_ce_exp")
    with r2[3]: nifty_pe_expiry  = st.selectbox("PE Expiry", _nf_exp_opts, key="nf_pe_exp")
...
with tab3:
    ...
    def expiry_to_date(s):
        ...
        # Text format: 26MAR12 or 2026MAR12
        pat1 = re.compile(r"(\d{2,4})([A-Z]{3})(\d{2})")
        m = pat1.match(s)
        if m:
            yy = int(m.group(1)) % 100
            mm = MONTHS.get(m.group(2), 1)
            dd = int(m.group(3))
            return date(2000 + yy, mm, dd)
        # Monthly text: 2026MAR (no day) — use last day of month
        pat2 = re.compile(r"(\d{2,4})([A-Z]{3})$")
        m = pat2.match(s)
        if m:
            import calendar
            yy = int(m.group(1)) % 100
    ...
    _iv_expiries = get_expiries()
    _iv_sx_opts  = _iv_expiries.get("SENSEX", []) or ["260312"]
    _iv_nf_opts  = _iv_expiries.get("NIFTY",  []) or ["260310"]
    with iv_r0[2]: sx_iv_expiry = st.selectbox("Sensex Expiry", _iv_sx_opts, key="iv_sx_exp")
    with iv_r0[3]: nf_iv_expiry = st.selectbox("Nifty Expiry",  _iv_nf_opts, key="iv_nf_exp")
