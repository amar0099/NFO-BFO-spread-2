...
with tab2:
    ...
    with c_row[2]:
        _cc1, _cc2, _cc3 = st.columns([1.2, 1, 2])
        with _cc1: c_auto  = st.checkbox("Auto Refresh", value=False, key="c_auto_refresh")
        with _cc2: c_secs  = st.slider("Sec", 5, 60, 10, key="c_refresh_secs")
        with _cc3: custom_fetch = st.button("⟳  FETCH 4-LEG DATA", type="primary", key="c_fetch", use_container_width=True)
    ...
    if custom_fetch or (st.session_state.get("c_auto_refresh") and st.session_state.get("_c_rerun")):
        fyers = get_fyers_client()
        if fyers is None:
    ...
    # Auto refresh for butterfly tab
    if st.session_state.get("c_auto_refresh") and not st.session_state.df_custom.empty:
        if custom_date_str == date.today().strftime("%Y-%m-%d"):
            time.sleep(st.session_state.get("c_refresh_secs", 10))
            st.session_state["_c_rerun"] = True
            st.rerun()
    st.session_state["_c_rerun"] = False
