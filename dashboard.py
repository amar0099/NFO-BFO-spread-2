...
def fetch_live_data():
    ...
    # Deduplicate index on each dataframe (loop var reassignment doesn't work — must do explicitly)
    ...
    df_sx_spot = df_sx_spot[~df_sx_spot.index.duplicated(keep="last")] if not df_sx_spot.empty else df_sx_spot
    df_nf_spot = df_nf_spot[~df_nf_spot.index.duplicated(keep="last")] if not df_nf_spot.empty else df_nf_spot

    # Align all to common index before building DataFrame
