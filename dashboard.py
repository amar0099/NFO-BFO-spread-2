...
if df.empty:
    ...
    # ── HIGH / LOW lines for CE and PE spread ──
    ce_high = df["ce_spread"].max()
    ce_low  = df["ce_spread"].min()
    pe_high = df["pe_spread"].max()
    pe_low  = df["pe_spread"].min()

    fig.add_hline(y=ce_high, line_dash="dot", line_color="#ff4444", line_width=1, opacity=0.5,
        annotation_text=f"CE H: {ce_high:.0f}", annotation_position="right",
        annotation_font=dict(color="#ff4444", size=10), row=1, col=1)
    fig.add_hline(y=ce_low, line_dash="dot", line_color="#ff4444", line_width=1, opacity=0.5,
        annotation_text=f"CE L: {ce_low:.0f}", annotation_position="right",
        annotation_font=dict(color="#ff4444", size=10), row=1, col=1)
    fig.add_hline(y=pe_high, line_dash="dot", line_color="#44ff88", line_width=1, opacity=0.5,
        annotation_text=f"PE H: {pe_high:.0f}", annotation_position="left",
        annotation_font=dict(color="#44ff88", size=10), row=1, col=1)
    fig.add_hline(y=pe_low, line_dash="dot", line_color="#44ff88", line_width=1, opacity=0.5,
        annotation_text=f"PE L: {pe_low:.0f}", annotation_position="left",
        annotation_font=dict(color="#44ff88", size=10), row=1, col=1)
    ...
    if show_diff:
        ...
        fig.add_hline(y=diff_high, line_dash="dot", line_color="#ffaa00", line_width=1, opacity=0.5,
            ...
            annotation_font=dict(color="#ffaa00", size=10), row=2, col=1)
    ...
    if show_diff:
        fig.update_yaxes(gridcolor=T["grid"], title_text="4 Leg", tickfont=dict(size=10),
            showspikes=True, spikemode="across", spikesnap="cursor",
            spikecolor=T["text3"], spikethickness=1, spikedash="dot", row=2, col=1)
        fig.update_xaxes(gridcolor=T["grid"], tickfont=dict(size=10),
            showspikes=True, spikemode="across", spikesnap="cursor",
            spikecolor=T["text3"], spikethickness=1, spikedash="dot", row=2, col=1)
