    ...
    try:
        ...
        for yy, mm, dd, mon in parsed:
            ...
            if is_monthly:
                ...
                code  = f"{yy:02d}{mm}{dd:02d}"    # 260325 → but Fyers needs YYM(no-zero)DD
                code  = f"{yy:02d}{mm}{dd:02d}"
...
def expiry_selectbox(label, opts_dict, manual_key, select_key, default_manual):
    """opts_dict = {label: fyers_code}. Returns fyers_code of selected."""
    if opts_dict:
        labels = list(opts_dict.keys())
        sel = st.selectbox(label, labels, key=select_key)
        return opts_dict[sel]   # ← return the Fyers code directly
...
with tab3:
    ...
    def expiry_to_date(s):
        ...
        m = _re.match(r"(\d{2})([A-Z]{3})$", s)   # monthly: 26MAR
        if m:
