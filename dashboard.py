...
# Expiry store: maps display label → Fyers symbol code
# e.g. "25 Mar (W)" → "26325"  or  "31 Mar (M)" → "26MAR"
_EXPIRY_CODE = {}  # populated by fetch_expiries_for

_MONTHS = ["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"]

def _parse_expiry_entry(date_str, epoch_str):
    """Parse date string like '25-03-2026' into (display_label, fyers_code)."""
    ...
    try:
        ...
        yy = yyyy % 100
        mon = _MONTHS[mm - 1]
        return (f"{dd:02d} {mon} {yy:02d}", yy, mm, dd, mon)
    except Exception:
        # fallback: parse from epoch
        import datetime as _dt
        dt = _dt.datetime.fromtimestamp(int(epoch_str), tz=_dt.timezone.utc)
        return (dt.strftime("%d %b %y").upper(), dt.year % 100, dt.month, dt.day, _MONTHS[dt.month-1])

@st.cache_resource
def fetch_expiries_for(token, fyers_sym):
    """Returns list of display labels. Populates _EXPIRY_CODE as side effect."""
    try:
        cid = get_secret("FYERS_CLIENT_ID") or CLIENT_ID
        ...
        # Parse all entries: (display, yy, mm, dd, mon)
        parsed = []
        for entry in raw:
            if isinstance(entry, dict):
                d = entry.get("date", "")
                e = entry.get("expiry", "0")
                parsed.append(_parse_expiry_entry(d, e))

        # Detect monthly: last expiry of each calendar month
        from collections import defaultdict
        by_month = defaultdict(list)
        for item in parsed:
            _, yy, mm, dd, mon = item
            by_month[(yy, mm)].append(dd)
        for (yy, mm), days in by_month.items():
            by_month[(yy, mm)] = max(days)  # last day = monthly expiry

        labels = []
        for display, yy, mm, dd, mon in parsed:
            is_monthly = (dd == by_month[(yy, mm)])
            if is_monthly:
                fyers_code = f"{yy:02d}{mon}"          # e.g. 26MAR
                label = f"{display} (M)"
            else:
                fyers_code = f"{yy:02d}{mm}{dd:02d}"   # e.g. 260325
                label = f"{display} (W)"
            _EXPIRY_CODE[label] = fyers_code
            labels.append(label)
        return labels
    except Exception:
        return []
...
def expiry_selectbox(label, opts, manual_key, select_key, default_manual):
    """Dropdown of fetched expiries only."""
    options = opts if opts else [default_manual]
    sel = st.selectbox(label, options, key=select_key)
    return sel  # caller passes through _EXPIRY_CODE lookup via build_symbol
...
def build_symbol(exchange, underlying, expiry, option_type, strike):
    ...
    # Look up the Fyers code from the display label (populated by fetch_expiries_for)
    fyers_code = _EXPIRY_CODE.get(expiry)
    if fyers_code:
        return f"{exchange}:{underlying}{fyers_code}{strike}{ot}"
    # Fallback: raw string — handle text (26MAR) or numeric (YYMMDD)
...
with tab1:
    ...
    with r1[2]: sensex_ce_expiry = expiry_selectbox("CE Expiry", _sx_opts, "sx_ce_man", "sx_ce_sel", "260312")
    with r1[3]: sensex_pe_expiry = expiry_selectbox("PE Expiry", _sx_opts, "sx_pe_man", "sx_pe_sel", "260312")
    ...
    with r2[2]: nifty_ce_expiry = expiry_selectbox("CE Expiry", _nf_opts, "nf_ce_man", "nf_ce_sel", "260310")
    with r2[3]: nifty_pe_expiry = expiry_selectbox("PE Expiry", _nf_opts, "nf_pe_man", "nf_pe_sel", "260310")
