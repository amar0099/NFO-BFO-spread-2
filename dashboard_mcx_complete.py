...
# ─────────────────────────────────────────────
with tab5:
    st.markdown("### ⛏️ MCX Option Spread")

    mcx_syms = ["SILVER","SILVERM","GOLD","GOLDM","CRUDEOIL","NATURALGAS","COPPER","ZINC","ALUMINIUM"]

    r0 = st.columns([1.2,1,1,1,1.5])
    with r0[0]:
        mcx_date = st.date_input("Date", value=default_date, key="mcx_date")
    with r0[1]:
        mcx_ratio = st.number_input("Ratio", value=1.0, step=0.1, key="mcx_ratio")
    with r0[2]:
        mcx_interval = st.selectbox("Interval", [1,3,5,10,15,30,60], index=2, key="mcx_interval")
    with r0[3]:
        mcx_auto = st.checkbox("Auto Refresh", value=False, key="mcx_auto")
    with r0[4]:
        st.button("⟳ FETCH MCX DATA", key="mcx_fetch", use_container_width=True)

    row = st.columns([0.25,0.8,0.8,0.8,0.7,0.7,0.15,0.25,0.8,0.8,0.8,0.7,0.7])

    row[0].markdown("**LEG 1**")
    with row[1]:
        mcx1_under = st.selectbox("Underlying", mcx_syms, key="mcx1_under")
    with row[2]:
        mcx1_exp = st.text_input("Expiry", "26JUN", key="mcx1_exp")
    with row[3]:
        mcx1_opt = st.selectbox("Type", ["CE","PE"], key="mcx1_opt")
    with row[4]:
        mcx1_strike = st.number_input("Strike", value=230000, step=100, key="mcx1_strike")
    with row[5]:
        mcx1_mult = st.number_input("Mult", value=1.0, step=0.1, key="mcx1_mult")

    row[7].markdown("**LEG 2**")
    with row[8]:
        mcx2_under = st.selectbox("Underlying", mcx_syms, key="mcx2_under")
    with row[9]:
        mcx2_exp = st.text_input("Expiry", "26JUN", key="mcx2_exp")
    with row[10]:
        mcx2_opt = st.selectbox("Type", ["CE","PE"], key="mcx2_opt")
    with row[11]:
        mcx2_strike = st.number_input("Strike", value=230000, step=100, key="mcx2_strike")
    with row[12]:
